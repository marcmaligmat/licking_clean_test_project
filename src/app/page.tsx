...
import { Provider, Booking } from '@/lib/supabase'
  ...
  async function fetchProvider() {
    try {
      ...
      setProvider({
        ...
        bio: "Professional house cleaner with 8+ years experience. Trusted by 200+ families for deep cleaning, weekly maintenance, and move-in/out services. Eco-friendly products and satisfaction guaranteed.",
        rating: 5
      })
    } catch (error) {
    ...
    <div className="min-h-screen bg-crisp-white py-4 px-4 sm:py-8">
      <div className="max-w-profile mx-auto">
        ...
        <header className="text-center mb-8">
          ...
          <p className="text-body text-slate-gray">Top-Rated • Trusted • Reliable</p>
        </header>
                ...
                onError={(e) => {
                  ...
                  target.parentElement!.innerHTML = `
                    <div class="w-full h-full bg-soft-teal flex items-center justify-center text-white font-poppins text-2xl">
                      ${provider?.name?.split(' ').map(n => n[0]).join('') || 'MC'}
                    </div>
                  `
                }}
              />
            ...
            <div className="flex-1 text-center sm:text-left">
              ...
              <div className="flex items-center justify-center sm:justify-start gap-1 mb-3">
                {[...Array(5)].map((_, i) => (
                  <span 
                    key={i} 
                    className={`text-lg ${i < (provider?.rating || 5) ? 'text-yellow-400' : 'text-gray-300'}`}
                  >
                    ★
                  </span>
                ))}
